"""
title: OpenPipe Universal Reporting (Raw Output Capture)
author: Cline & Gwyn
version: 3.0.0
...
"""
OpenPipe Reporting Filter v3.0 - Universal with UI Toggle

Enhanced filter that works with ALL models and provides UI controls for OpenPipe reporting.
Includes toggle switch, custom icon, and real-time feedback for better user experience.

Authors: Cline (AI Assistant) & Gwyn (Human Collaborator) 
Version: 3.0.0 - Universal with UI Toggle
...
class Filter:
    """
    Universal OpenPipe Reporting Filter with UI Toggle
    
    Reports AI interactions from ALL models to OpenPipe with:
    - UI toggle switch for user control
    - Custom OpenPipe icon
    - Real-time status feedback
    - Enhanced raw response capture
    ...
    class Valves(BaseModel):
        ...
        ENABLE_DPO_DETECTION: bool = Field(
            default=True,
            description="Detect message regeneration and create DPO preference pairs"
        )
        AUTO_SUBMIT_DPO_PAIRS: bool = Field(
            default=False,
            description="Automatically submit DPO preference pairs to OpenPipe (requires DPO dataset)"
        )
        DPO_DATASET_NAME: str = Field(
            default="OpenWebUI DPO Training",
            description="Dataset name for DPO preference pairs"
        )
    ...
    def __init__(self):
        ...
        # Custom OpenPipe Icon (SVG Data URI)
        ...
        # Log initialization
        self._log("OpenPipe Filter v3.0 initialized successfully")
        self._log(f"Toggle enabled: {self.toggle}")
        # Note: API key will be configured via Valves after initialization
        self._log("Ready for API key configuration via Valves")
        
    def _log(self, message: str, level: str = "INFO"):
        """Debug logging helper with enhanced verbosity"""
        # Always log INFO and above for debugging
        import traceback
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        print(f"[{timestamp}] [OpenPipe Filter v3.0 {level}] {message}")
        if level == "ERROR":
            print(f"[{timestamp}] [OpenPipe Filter v3.0 TRACE] {traceback.format_exc()}")
    ...
    def _classify_interaction(self, body: Dict[str, Any]) -> str:
        ...
        Args:
            body: Request body containing metadata
    ...
    async def _send_to_openpipe(self, payload: Dict[str, Any]):
        ...
        try:
            ...
            if status_code == 200:
                ...
                self._log(f"Request payload preview: {json.dumps(payload, indent=2)[:1000]}...", "ERROR")
    ...
    async def inlet(self, body: Dict[str, Any], __event_emitter__, user: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Capture incoming request data and provide UI feedback
        ...
        await __event_emitter__({
            "type": "status",
            "data": {
                "description": f"🔄 Capturing {interaction_type} for OpenPipe",
                "done": False,
                "hidden": False,
            },
        })
    ...
    def _extract_raw_content_from_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """Extract content from a message in raw format"""
        if not isinstance(message, dict):
            return {"role": "assistant", "content": str(message) if message else ""}
            
        # Preserve message structure with tool calls
        if "role" in message and "content" in message:
            ...
            # Preserve tool calls in their raw format
            if "tool_calls" in message:
                raw_message["tool_calls"] = message["tool_calls"]
            if "function_call" in message:
                raw_message["function_call"] = message["function_call"]
        ...
        return {
            "role": message.get("role", "assistant"),
            "content": str(message.get("content", "")) if message.get("content") else ""
        }

    async def outlet(self, body: Dict[str, Any], __event_emitter__, user: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Process completed response and send to OpenPipe with UI feedback
        ...
        try:
            ...
            # Extract the most recent message (assistant response) in raw format
            messages = body.get("messages", [])
            if messages:
                response_message = messages[-1]
                raw_response_message = self._extract_raw_content_from_message(response_message)
            else:
                raw_response_message = {"role": "assistant", "content": ""}
            ...
            metadata = {
                ...
                "openwebui_version": "filter_v3.0_universal",
            ...
            await __event_emitter__({
                "type": "status",
                "data": {
                    "description": f"📤 Reporting {interaction_type} to OpenPipe ({inference_time_ms}ms)",
                    "done": False,
                    "hidden": False,
                },
            })
            ...
            await __event_emitter__({
                "type": "status",
                "data": {
                    "description": f"✅ Reported to OpenPipe successfully",
                    "done": True,
                    "hidden": False,
                },
            })
            
            self._log(f"Successfully reported {interaction_type} (took {inference_time_ms}ms)")
